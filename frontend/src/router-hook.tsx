...
class RouterHook extends Logger {
  ...
  private renderedComponents = new Map<EUIMode, ReactElement<any>[]>([
    [EUIMode.GamePad, []],
    [EUIMode.Desktop, []],
  ]);
  private Route: any;
  ...
  addPatch(path: string, patch: RoutePatch, uiMode: EUIMode = EUIMode.GamePad) {
    return this.routerState.addPatch(path, patch, uiMode);
  }

  addGlobalComponent(name: string, component: FC, uiMode: EUIMode = EUIMode.GamePad) {
    this.globalComponentsState.addComponent(name, component, uiMode);
  }

  removeGlobalComponent(name: string, uiMode: EUIMode = EUIMode.GamePad) {
    this.globalComponentsState.removeComponent(name, uiMode);
  }

  removePatch(path: string, patch: RoutePatch, uiMode: EUIMode = EUIMode.GamePad) {
